...
class MoonsGeneralizationEvaluator:
    def __init__(self, device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.device = device
        self.generator = SyntheticDataGenerator(img_size=64, n_samples=300, noise=0.1)
    ...
    def run_generalization_test(self):
        ...
        # 生成测试数据（双月数据）
        print("生成测试数据（双月数据集）...")
        test_data = self.generate_moons_dataset(50)
    ...
    def visualize_results(self):
        ...
        # 测试损失分布
        axes[0, 1].hist(self.results['model_a']['test_losses'], alpha=0.7, label='Model A', color='blue', bins=20)
        axes[0, 1].hist(self.results['model_b']['test_losses'], alpha=0.7, label='Model B', color='red', bins=20)
        ...
        plt.tight_layout()
        plt.savefig('moons_generalization_results.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def save_results(self):
        ...
        with open('moons_generalization_results.json', 'w') as f:
            json.dump(results_summary, f, indent=2)
        
        print(f"\n结果已保存到:")
        print(f"- moons_generalization_results.png (可视化图表)")
        print(f"- moons_generalization_results.json (详细数据)")


def main():
    """主函数"""
    evaluator = MoonsGeneralizationEvaluator()
